...
const ProductChart: React.FC<ProductChartProps> = ({ results, suppliers, filters, selectedFilter }) => {
  ...
  const dataBySupplier = useMemo(() => groupMonthlyByEntity(filteredMonthlyData), [filteredMonthlyData])

  const dailyDataBySupplier = useMemo<Record<number, Record<string, DailyFinancial[]>>>(() => {
        ...
        <div className="bg-(--color-bg-surface) p-4 rounded-lg border border-(--color-border) shadow-xs xl:col-span-2">
          ...
          <ResponsiveContainer width="100%" height={isDesktop ? CHART_HEIGHTS.xl : CHART_HEIGHTS.large}>
            <BarChart data={monthlyDataArray} margin={CHART_MARGINS.withBottomLabels}>
              ...
              <Legend />
              <Bar dataKey="Total" name="Total Compras" fill={CHART_COLORS.total}>
