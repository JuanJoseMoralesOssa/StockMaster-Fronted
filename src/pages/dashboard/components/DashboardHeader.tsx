...
function DashboardHeader({ title, subtitle, dateFormatted }: Readonly<DashboardHeaderProps>) {
  return (
    <div className="flex flex-col md:flex-row md:items-start justify-between mb-7 gap-4">
      ...
      <div className="text-[12.5px] text-(--color-text-secondary) bg-(--color-bg-surface) border border-(--view-accent-border,var(--color-border)) rounded-lg px-3 py-1.5 font-mono shadow-xs shrink-0">
        {dateFormatted}
      </div>
    </div>
  )
