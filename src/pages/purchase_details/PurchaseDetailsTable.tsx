...
export default function PurchasesDetailsTable({
    ...
    const [purchasesLength, setPurchasesLength] = useState(details.length ?? 0)
    const {
        products,
    ...
    const { productSummary } = usePurchaseSummary(details)

    useEffect(() => {
        setPurchasesLength(details.filter((row) => !row.toDelete).length)
    }, [details])

    const addPurchase = (e: React.FormEvent) => {
        e.preventDefault()
        const newPurchases = [
            ...details,
            {
    ...
    const updatePurchase = (id: number, field: string, value: PurchaseUpdateValue) => {
        const newPurchases = details.map((row) => {
            if (row.id === id) {
    ...
    return (
        <div className='px-2 py-1 min-w-2xl flex flex-col items-center justify-center w-fit overflow-x-auto sm:overflow-visible'>
            ...
            {/* Tablas de resumen */}
            {productSummary.length > 0 && purchasesLength > 0 && (
