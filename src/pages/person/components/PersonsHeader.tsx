import { useState } from 'react'
import { Plus } from 'lucide-react'
import { Dialog } from '../../components/dialog/Dialog'
import { DialogTrigger } from '../../components/dialog/DialogTrigger'
import { DialogContent } from '../../components/dialog/DialogContent'
import { DialogHeader } from '../../components/dialog/DialogHeader.'
import { DialogTitle } from '../../components/dialog/DialogTitle'
import { DialogDescription } from '../../components/dialog/DialogDescription'
import PersonCreate from './PersonCreate'
import Person from '../../../types/Person'

interface PersonsHeaderProps {
    onPersonCreated: (newPerson: Person) => void
}

function PersonsHeader({ onPersonCreated }: Readonly<PersonsHeaderProps>) {
    const [open, setOpen] = useState(false)
    return (
        <section className='flex items-center justify-between gap-4 p-2 mr-10 md:mr-5 max-w-fit'>
            <section>
                <h2 className='text-3xl font-bold tracking-tight'>Proveedores</h2>
            </section>
            <Dialog className='relative'>
                <DialogTrigger setOpen={setOpen}>
                    <section className='flex items-center justify-center gap-2 p-2 border rounded-lg text-white bg-blue-500 border-gray-50 hover:bg-gray-600 hover:border-gray-800'>
                        <Plus className='md:mr-2 h-4 w-4' />
                        <p>Nuevo Proveedor</p>
                    </section>
                </DialogTrigger>
                <DialogContent
                    open={open}
                    setOpen={setOpen}
                    className='sm:max-w-[600px] m-auto'>
                    <DialogHeader>
                        <DialogTitle>Crear Nuevo Proveedor</DialogTitle>
                        <DialogDescription>
                            Completa los detalles del proveedor. Todos los campos son
                            requeridos.
                        </DialogDescription>
                    </DialogHeader>
                    <PersonCreate
                        onSuccess={() => setOpen(false)}
                        onPersonCreated={onPersonCreated}
                    />
                </DialogContent>
            </Dialog>
        </section>
    )
}

export default PersonsHeader
